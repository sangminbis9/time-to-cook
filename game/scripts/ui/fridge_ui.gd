...
##
## 냉장고 슬롯만 팝업으로 띄우고, 플레이어 쪽은 HUD 하단 인벤토리 바를
## 그대로 쓴다 — HUD가 바 클릭을 on_inventory_slot_clicked로 넘겨준다.
##
...
## 커서 상태(들기/내려놓기)가 바뀔 때 — HUD가 하단 바를 다시 칠한다
...
func _ready() -> void:
	...
	root.add_child(_make_fridge_grid(GameServer.fridge.slots.size()))

	var hint: Label = Label.new()
	hint.text = "하단 인벤토리와 클릭: 이동/교환\nShift+클릭: 빠른 이동   Esc: 닫기"
...
## 냉장고 슬롯 줄 — HUD 하단 바와 같은 40px 규격
func _make_fridge_grid(count: int) -> HBoxContainer:
	...
	for i in range(count):
		...
		button.pressed.connect(_on_slot_clicked.bind(ZONE_FRIDGE, i))
...
## HUD가 하단 인벤토리 바 클릭을 넘겨준다
func on_inventory_slot_clicked(slot: int) -> void:
	_on_slot_clicked(ZONE_INV, slot)


## 커서에 들려 있는 인벤토리 슬롯 (HUD 반투명 표시용; 없으면 -1)
func cursor_inv_slot() -> int:
	return _cursor_slot if _cursor_zone == ZONE_INV else -1
...
func _refresh() -> void:
	for i in range(_fridge_buttons.size()):
		_paint_slot(_fridge_buttons[i], i)


func _paint_slot(button: TextureButton, slot: int) -> void:
	button.modulate = Color.WHITE
	var icon: TextureRect = button.get_child(0) as TextureRect
	var iid: int = _slot_iid(ZONE_FRIDGE, slot)
	if ZONE_FRIDGE == _cursor_zone and slot == _cursor_slot:
		# 커서에 든 원본 슬롯은 반투명 표시
